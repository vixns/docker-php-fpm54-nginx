...
RUN apt-get update && apt-get install -y ca-certificates curl gnupg librecode0 libsqlite3-0 libxml2 --no-install-recommends && rm -r /var/lib/apt/lists/*
...
echo "deb http://http.debian.net/debian stretch contrib non-free main" >> /etc/apt/sources.list && \
echo "deb http://http.debian.net/debian stretch-backports contrib non-free main" >> /etc/apt/sources.list && \
apt-get update && apt-get --no-install-recommends -t stretch -y dist-upgrade
RUN apt-get install --no-install-recommends -t stretch-backports -y ca-certificates nginx runit file re2c libicu-dev zlib1g-dev libmcrypt-dev libmagickcore-6.q16-dev libmagickcore-dev libmagickwand-dev libmagick++-dev libmagick++-6.q16-dev libjpeg-dev libpng-dev libicu52 libmcrypt4 g++ imagemagick git libssl-dev xfonts-base xfonts-75dpi libfreetype6-dev && \
...
RUN dpkg --purge libicu-dev libxml2-dev libmagickcore-dev libmagickwand-dev libmagick++-dev libssl-dev libfreetype6-dev libmagickcore-6.q16-dev libgraphviz-dev libglib2.0-dev libtiff5-dev libwmf-dev libcairo2-dev libgdk-pixbuf2.0-dev libfontconfig1-dev librsvg2-dev libmagickwand-6.q16-dev libmagick++-6.q16-dev libmagickcore-6.q16-dev && \
